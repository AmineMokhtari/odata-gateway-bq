...
# ==============================================================================
# obq-hub Publish Script (Bash)
# ==============================================================================
# This script builds and publishes the `obq-hub` Docker image using GCP 
...
# The `obq-hub` service depends on the shared `common/` package located at 
# the root of the monorepo. Therefore, the Docker build context must be the 
# root directory (`..`), rather than the `obq-hub` directory itself.
# ==============================================================================
set -e
...
show_help() {
    ...
    echo "Builds and publishes the obq-hub Docker image to GCP Artifact Registry via Cloud Build."
    echo ""
    echo "Options:"
...
# --config: Points to the cloudbuild.yaml file located inside the obq-hub folder
# --substitutions: Passes the computed image path into the cloudbuild.yaml step
gcloud builds submit --project "${PROJECT_ID}" --config obq-hub/cloudbuild.yaml --substitutions "_IMAGE_PATH=${IMAGE_PATH}" .
